pipeline {
  ...
  environment {
    ...
    EXT_PIP = 'none'
    EXT_BLOB = 'none'
    BUILD_VERSION_ARG = 'MARASCHINO_COMMIT'
    LS_USER = 'thelamer'
    LS_REPO = 'docker-maraschino'
    DOCKERHUB_IMAGE = 'lspipelive/maraschino'
    DEV_DOCKERHUB_IMAGE = 'lspipetest/maraschino'
    PR_DOCKERHUB_IMAGE = 'lspipepr/maraschino'
    ...
    /* ###############
     ...
     stage('Build') {
       steps {
           ...
           sh "docker build --pull --no-cache -t ${DOCKERHUB_IMAGE}:${EXT_RELEASE}-ls${LS_TAG_NUMBER} \
